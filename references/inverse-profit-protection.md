...
- 부분익절 완료는 주문 생성/제출이 아니라 다음 KIS 잔고의 실제 수량 감소로만 확인한다. 부분체결 재주문량은 단계 목표 누적수량에서 실제 잔고 감소량을 뺀 잔량으로 제한한다.
...
- tracker의 읽기·고점/수량 갱신·저장 전체를 교차 프로세스 `flock`으로 직렬화하고, snapshot은 임시 파일 write/fsync → atomic rename → directory fsync 순서로 저장한다.
