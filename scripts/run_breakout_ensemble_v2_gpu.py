...
PANEL = ROOT / "reports/backtests/pit_random500_2022-01-01_2026_ohlcv_panel.csv"
OUT_JSON = ROOT / "reports/backtests/breakout_ensemble_v3_pit.json"
OUT_MD = ROOT / "reports/backtests/breakout_ensemble_v3_pit.md"
BASE_COST = 0.0013
...
def gpu_shift(t: torch.Tensor, periods: int) -> torch.Tensor:
    """Shift along time axis by periods (positive = future, negative = past)."""
    if periods == 0:
        return t.clone()
    T, N = t.shape
...
def compute_episode_returns_gpu(open_t: torch.Tensor, signals: np.ndarray, hold: int) -> torch.Tensor:
    """Compute episode returns [num_signals, N] on GPU."""
    T, N = open_t.shape
    num_sig = len(signals)
    ...
    # Cap extreme positive
    daily_ret = daily_ret.clamp(min=-1.0, max=0.30)

    episode_ret = torch.zeros(num_sig, N, device=DEVICE, dtype=open_t.dtype)
    for i, sig in enumerate(signals):
        start = int(sig) + 1
        stop = int(sig) + hold + 1
        if stop > daily_ret.shape[0]:
            stop = daily_ret.shape[0]
        if start >= stop:
            continue
        # Product of (1 + daily_ret) over holding period
...
def evaluate_gpu(
    ...
    """Full evaluation pipeline on GPU."""
    ...
    for i, sig_row in enumerate(selected_rows):
        ...
        gross = torch.where(valid, episode_ret[sig_row, idx], torch.tensor(0.0, device=DEVICE)).sum() / top_k
        returns[i] = gross
        executed_slots += int(valid.sum().item())

        if per_side_cost > 0:
            if i == 0:
                returns[i] -= valid.sum().float() / top_k * per_side_cost
            else:
                sides = traded_sides_gpu(prev_idx, prev_valid, idx, valid)
                returns[i] -= sides / top_k * per_side_cost

        prev_idx = idx
        prev_valid = valid

    if per_side_cost > 0 and len(selected_rows) > 0:
        returns[-1] -= prev_valid.sum().float() / top_k * per_side_cost
...
def build_ml_features(g: dict) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    # Forward return: open-to-open over 5 days (primary target)
    open_t = g["open_t"]
    fwd_ret = open_t[5:] / open_t[:-5] - 1.0
    fwd_ret = torch.where(torch.isnan(fwd_ret), torch.tensor(0.0, device=DEVICE), fwd_ret)
    fwd_ret = fwd_ret.clamp(min=-1.0, max=0.30)

    # Align: features up to T-5, target from row 0
    feat_aligned = features[:-5]
...
def main() -> None:
    ...
    train_cutoff = close.index.searchsorted(pd.Timestamp("2024-12-31"))
    if train_cutoff >= len(close.index):
        train_cutoff = len(close.index) - 1
    ...
    valid_torch = ~torch.isnan(feat).any(dim=2) & ~torch.isnan(target) & mask_mlp
    ...
    g["scores"]["lightgbm_gpu"] = gpu_rank_pct_vectorized(deep_full)  # keep name for compat
    g["gates"]["lightgbm_gpu"] = ~torch.isnan(g["scores"]["lightgbm_gpu"])
    print(f"done ({time.time()-t0:.2f}s)")
    ...
    attn_model = AttnNet(F_dim, d_model=32, n_heads=4).to(DEVICE)
    with torch.enable_grad():
        ...
        for epoch in range(25):
            for xb, yb in loader:
                ...
                opt3.step()
            sched3.step()
    attn_pred = attn_model(feat_norm_all)
    ...
    g["scores"]["xgboost_gpu"] = gpu_rank_pct_vectorized(attn_full)  # keep name for compat
    g["gates"]["xgboost_gpu"] = ~torch.isnan(g["scores"]["xgboost_gpu"])
    print(f"done ({time.time()-t0:.2f}s)")
    ...
    for hold in HOLDS:
        ...
        for model in ALL_MODELS:
            ...
            _, rets, _ = evaluate_gpu(
                ...
                compute_episode_returns_gpu(g["open_t"], signals, hold),
                signals, test_rows, 10, hold, BASE_COST
            )
    ...
    report = {
        ...
        "promotion": {
            ...
            "verdict": "RESEARCH_ONLY_V2",
            "v2_improvements": [
                "10 diverse factor families (not just breakout variants)",
                "3 ML models: LightGBM, XGBoost, torch MLP (MPS)",
                "4 ensemble variants including uncorrelated selection",
                "GPU-accelerated factor/return/bootstrap computation",
                "5000-sample bootstrap confidence intervals",
